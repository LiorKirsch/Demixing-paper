...
We determine the values of $\lambda_{r,s}$ in two steps. First, as a preprocessing step, we used a brain region hierarchy (\figref{tree}) to compute the graph distances between every two regions $d_{r,s}$. {\bf{We choose the score as the level of the tree for their closest common parent. For example the frontal lobe and the occipital lobe are both a part of the cerebral cortex so their score will be $e^3$. The cerebellum on the occipital lobe are joined only at the root so their score is $e^1$. }}
